...
async def pipeline_run(
    ...
    face_task = asyncio.to_thread(verify_face, f_bytes, l_bytes, logs, faceThreshold)

    # Gather independent forensic modules concurrently
    ocr_res, tamper_res, face_res = await asyncio.gather(ocr_task, tamper_task, face_task)

    liveness_res = face_res.get("liveness", {
        ...
        "ear_score": 0.0,
        "head_yaw_deg": 0.0,
        "detail": "No live selfie capture provided"
    })
